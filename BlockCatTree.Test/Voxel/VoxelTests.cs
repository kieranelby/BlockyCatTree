...
public class VoxelTests
{
    ...
    public void TestBasicOperations()
    {
        ...
        Assert.That(_uut.GetInclusiveZedBounds(), Is.EqualTo(new ZedBounds(Zed.Origin, Zed.Origin)));
        ...
        Assert.That(_uut.GetInclusiveZedBounds(), Is.EqualTo(new ZedBounds(_pointA1.Zed, _pointB2.Zed)));
        _uut.Remove(_pointA1);
        Assert.That(_uut.Get(_pointA1), Is.EqualTo(null));
        Assert.That(_uut.IsEmpty, Is.False);
        Assert.That(_uut.GetInclusiveZedBounds(), Is.EqualTo(new ZedBounds(_pointB1.Zed, _pointB2.Zed)));
    ...
    public void TestSlicing()
    {
        ...
        _uut.Set(_pointA1, 1);
        _uut.Set(_pointB1, 2);
        var bounds = _uut.GetInclusiveZedBounds();
        Assume.That(bounds, Is.EqualTo(new ZedBounds(new Zed(3), new Zed(6))));
        foreach (var zed in bounds)
        {
